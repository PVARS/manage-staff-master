    ...
    if (isset($param['registFlg']) && $param['registFlg'] == 1){
        ...
        if ($error == 0){
            if (isset($param['mode']) && $param['mode'] == 'update'){
                updateMember($con, $param);
            }
            insertMember($con, $param);
        }
    }
}
...
if (isset($param['uid']) && !empty($param['uid'])){
    $readonly = 'readonly';
    $dataMember = getMemberById($con, $param);
...
function getMemberById($con, $param){
    $data = [];
    $recCnt = 0;
...
function updateMember($con, $param){
    if (!empty($param['birthday'])){
...
function insertMember($con, $param){
    if (!empty($param['birthday'])){
        $birthday = strtotime($param['birthday']);
    } else $birthday = 'NULL';
